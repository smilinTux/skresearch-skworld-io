...
- **Two sovereign libraries:** **`sk_pqc`** (Dart) — web (noble) + native (liboqs), delivers hybrid PQC **in the browser** (where no WebCrypto PQC API exists), CDP-verified in-page DMs. **`sk_pgp`** (Python, PyO3→Sequoia) — the **PGPy replacement** and the *engine* that lets the Python services sign with v6/PQC keys at all (PGPy can't parse v6, gpg can't sign PQC); in-process ML-DSA-87+Ed448 generate/sign/verify proven, self-contained wheel.
...
- Libraries: `sk_pqc` (Dart/Flutter KEM, Apache-2.0, public) · live at skpqc.skworld.io; `sk_pgp` (Python OpenPGP-PQC, PyO3→Sequoia, Apache-2.0) · github.com/smilinTux/sk_pgp
