...
*Draft — June 2026 · Apache-2.0 libraries, **now published to the public registries**: the `sk_pqc` hybrid KEM ships across Python, Dart, and Rust behind one wire format — [PyPI `sk-pqc`](https://pypi.org/project/sk-pqc/) (`pip install sk-pqc`) · [pub.dev `sk_pqc`](https://pub.dev/packages/sk_pqc) (`dart pub add sk_pqc`) · [crates.io `sk-pqc`](https://crates.io/crates/sk-pqc) (`cargo add sk-pqc`) — all import `sk_pqc`; repos [sk-pqc-py](https://github.com/smilinTux/sk-pqc-py) · [sk-pqc-dart](https://github.com/smilinTux/sk-pqc-dart) · [sk-pqc-rs](https://github.com/smilinTux/sk-pqc-rs) — and `sk_pgp` (Python OpenPGP-PQC) at [github.com/smilinTux/sk_pgp](https://github.com/smilinTux/sk_pgp). **A Level-3 forward-secret hybrid-PQ DM ratchet is proven live cross-node between two physical machines** (lumina@.158 ↔ jarvis@.41, over tailscale federation). Live at [skpqc.skworld.io](https://skpqc.skworld.io)*
...
- **Sovereign / attributable** — a **capauth DID + FQID**, with **hybrid Ed25519+ML-DSA-65** signed prekeys and an identity signature on session establishment. Federated, non-repudiable, auditable — the enterprise / accountability posture.
- **Anonymous / deniable** — **no DID**, no capauth identity: pairwise **opaque queue ids** and a **deniable HMAC** (a symmetric MAC — authentic to the participants, repudiable to any third party, never a signature on content), with MITM resisted by an out-of-band/QR link whose secret rides in the URI hash fragment and never reaches the relay.
...
flowchart TD
    ...
    mode -->|"ANONYMOUS / deniable"| AN["🕶️ no DID · opaque queue ids<br/>deniable HMAC (repudiable)<br/>OOB / QR link exchange"]
...
- **A choice of identity posture over one PQC core.** **SimpleX** [15] pioneered no-identity messaging — opaque pairwise queue ids and deniable authentication, with no user-account graph — but keeps its routing/metadata layer classical. Signal, Apple PQ3, and MLS are account- or group-bound. We treat *attribution itself* as a runtime mode (§4.10): the same hybrid KEM and signature-free ratchet serve either a **sovereign capauth DID** or a **SimpleX-style anonymous queue** posture, chosen per-conversation, while the metadata layer is **hybrid-sealed** rather than classical. We are explicit that this switch is mid-implementation and that, on a small sovereign tailnet, the anonymity set is far smaller than a public relay pool — never Tor-grade.
...
- **Live cross-node Level-3 ratchet (two physical machines)**: the forward-secret hybrid-PQ DM ratchet ran between `lumina@192.168.0.158` and `jarvis@192.168.0.41` over tailscale federation — each `DmRatchetManager` sealed a DM the other opened, hybrid throughout, epoch ratchet advancing on both ends. Proven through the `dm_manager` orchestration **directly**; the end-to-end round-trip through the live production daemon is a **pending belt-and-suspenders** check, and the surface is reported as exactly that.
- **Three libraries published to public registries**: `sk_pqc` is live on PyPI, pub.dev, and crates.io (crates.io cut 2026-06-27) — all importing `sk_pqc`, one wire format, cross-implementation interop gated against recorded vectors.
- **One Rust core, in progress**: a PyO3 `sk-pqc-rs` build (`--features python` → abi3 `sk_pqc_rs` wheel, pure-Rust X25519+ML-KEM-768, no OpenSSL/liboqs) to back the Python binding from the *same* implementation as the Rust crate — wire format unchanged, parity-verified against Python-computed vectors.
...
- **The identity-mode switch (§4.10), wired live.** The anonymous-queue addressing primitive and the sovereign signed-prekey both exist in-tree; what remains is the `auth_mode` mode-flag plumbing at session establishment and enforcement of the two invariants (no silent sovereign→anonymous downgrade; signature-free ratchet in both modes), tracked under coord `c0f105a1`. It is default-OFF and reported as scaffold until it lands on the live transport.
- **The live-daemon round-trip** (belt-and-suspenders). The cross-node Level-3 DM ratchet is proven through the `dm_manager` seal/open path directly, machine-to-machine over tailscale; what remains is confirming the same exchange end-to-end through the production `skchat` daemon. The cryptographic claim is already met; this closes the operational loop and is reported as pending until it does.
- **One PyO3 Rust-core, all languages.** The `sk-pqc-rs --features python` abi3 wheel (`sk_pqc_rs`, pure-Rust X25519+ML-KEM-768) collapses the three parallel ML-KEM implementations behind the binding into a single vetted core selected by id — wire format frozen, parity-gated. In flight; the wheel builds, the cross-language vectors match.
...
15. SimpleX Chat, *SMP — Simplex Messaging Protocol* and *Private message routing* (no-identity queue addressing; v5.6 post-quantum / v5.8 private-routing design notes). [simplex.chat](https://simplex.chat/) · [github.com/simplex-chat/simplexmq](https://github.com/simplex-chat/simplexmq)
...
**Libraries** — `sk_pqc` (hybrid KEM + Level-3 DM/group ratchet, one wire format across Python/Dart/Rust), Apache-2.0, **published to public registries**: [PyPI `sk-pqc`](https://pypi.org/project/sk-pqc/) (`pip install sk-pqc`) · [pub.dev `sk_pqc`](https://pub.dev/packages/sk_pqc) (`dart pub add sk_pqc`) · [crates.io `sk-pqc`](https://crates.io/crates/sk-pqc) (`cargo add sk-pqc`, cut 2026-06-27) — all import `sk_pqc`; repos [sk-pqc-py](https://github.com/smilinTux/sk-pqc-py) · [sk-pqc-dart](https://github.com/smilinTux/sk-pqc-dart) · [sk-pqc-rs](https://github.com/smilinTux/sk-pqc-rs) · live at [skpqc.skworld.io](https://skpqc.skworld.io); `sk_pgp` (Python OpenPGP-PQC engine, PyO3→Sequoia), Apache-2.0, [github.com/smilinTux/sk_pgp](https://github.com/smilinTux/sk_pgp).
